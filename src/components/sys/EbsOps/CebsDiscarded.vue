...
<template>
    <div>
        <div class="modal fade" id="DiscardedSignals" aria-labelledby="ViewDataLabel" aria-hidden="true">
            <div class="modal-dialog modal-fullscreen">
                <div class="modal-content">
                    <div class="modal-header shadow-lg">
                        <div class="col-12">
                            <input type="text" class="form-control float-end mt-4" v-model="searchTerm"
                                placeholder="Search signals you discarded" />
                        </div>
                    </div>
                                    ...
                                    <div :id="'collapse' + index" class="collapse" :class="{ show: index == 0 }"
                                        ...
                                        <div class="card mt-2">
                                            ...
                                            <div class="card-body text-center">
                                                ...
                                                <button type="button"
                                                    class="btn btn-danger DeleteMe btn-sm ms-5" :data-id="record.id"
                                                    :data-tablename="tableName"
                                                    @click="ReverseDiscardedSignals(record.ReportID)">
                                                    <i class="fas fa-check"></i> 
                                                    Reverse Signal Discard
                                                </button>
                                            </div>
                                        </div>
                                    </div>
...
export default {
  data() {
    return {
      ...
      ignoredColumns: ["created_at", "updated_at", "SignalID", "ReportID", "UserID", "TriagingStatus", "HFID", "name", "email", "Email", "Role", "WardID", "Constituency", "ChvGroupID", "remember_token", "VillageID", "HealthFacilityID", "EbsSignalCategoryID"],
  ...
  methods: {
    fetchRecords() {
      ...
      axios
        .post(`${this.SERVER_URL}CebsDiscardedSignals`, {
          UserID: UserID,
    ...
    ReverseDiscardedSignals(ReportID) {
      Swal.fire({
        title: "Confirm Signal Discard Reversal",
        html: `
          Are you sure you want to reverse this signal's discard status
        `,
        icon: "warning",
        ...
        if (result.isConfirmed) {
          const UserID = sessionStorage.getItem("UserID");
          axios
            .post(window.SERVER_URL + "ReverseCebsDiscardedTriage", {
              UserID: UserID,
              ReportID: ReportID,
            })
            .then((response) => {
