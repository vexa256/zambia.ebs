...
<template>
    <div>
        <div class="modal fade" id="TriagedSignalsModal" aria-labelledby="ViewDataLabel" aria-hidden="true">
            <div class="modal-dialog modal-fullscreen">
                <div class="modal-content">
                    <div class="modal-header shadow-lg">
                        <div class="col-12">
                            <input type="text" class="form-control float-end mt-4" v-model="searchTerm"
                                placeholder="Search signals you triaged" />
                        </div>
                    </div>
                                        ...
                                        <div class="card mt-2">
                                            ...
                                            <div class="card-body text-center">
                                                ...
                                                <button type="button"
                                                    ...
                                                    @click="ReverseTriageSignal(record.ReportID)">
...
export default {
  ...
  methods: {
    fetchRecords() {
      ...
      axios
        .post(`${this.SERVER_URL}CebsTriagedSignals`, {
          UserID: UserID,
    ...
    ReverseTriageSignal(ReportID) {
      Swal.fire({
        title: "Confirm Signal Triage Reversal",
        html: `
          Are you sure you want to reverse this signal's triage status
        `,
        icon: "warning",
        ...
        if (result.isConfirmed) {
          const UserID = sessionStorage.getItem("UserID");
          axios
            .post(window.SERVER_URL + "ReverseCebsSignalTriage", {
              UserID: UserID,
              ReportID: ReportID,
            })
            .then((response) => {
