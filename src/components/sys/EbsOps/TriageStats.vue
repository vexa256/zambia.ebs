        ...
        <div class="table-responsive">
          <table class="table table-bordered">
            ...
            <tbody>
              <tr class="text-light">
                <td>Un-Triaged</td>
                <td>{{ unTriagedSignals }}</td>
              </tr>
              <tr class="text-light">
                <td>Triaged</td>
                <td>{{ triagedSignals }}</td>
              </tr>
              <tr class="text-light">
                <td>Discarded</td>
...
export default {
  ...
  methods: {
    fetchSignalStatistics() {
      ...
      axios
        .post(this.SERVER_URL + "SignalStats", { UserID })
        .then((response) => {
          const newUnTriagedSignals = response.data.UnTriaged;
          const newTriagedSignals = response.data.Triaged;
          const newDiscardedSignals = response.data.Discarded;

          if (
            newUnTriagedSignals !== this.unTriagedSignals ||
            newTriagedSignals !== this.triagedSignals ||
            newDiscardedSignals !== this.discardedSignals
          ) {
            this.unTriagedSignals = newUnTriagedSignals;
            this.triagedSignals = newTriagedSignals;
    ...
    drawBarChart() {
      new Chart("barChart", {
        type: "bar",
        data: {
          labels: ["Un-Triaged", "Triaged", "Discarded"],
          datasets: [
            {
              label: "Signal Statistics",
              data: [
                this.unTriagedSignals,
                this.triagedSignals,
